/*
https://www.codewars.com/kata/58c218efd8d3cad11c0000ef/csharp
...
namespace CodeWars
{
    public class SimpleFun194BinaryString
    {
        public int BinStr(string s)
        {
            return ("0" + s).Zip(s, (a, b) => a != b).Count(b => b);
            //return (Regex.Split(s, "1+0+").Length - 1) * 2 + s[^1] - 48;


            // int check = 0;
            // if (s.Contains('1')) s = s.Substring(s.IndexOf('1'));
            // while (s.Contains('1'))
            // {
            //     s = string.Concat(s.Select(x => x == '0' ? '1' : '0'));
            //     if (s.Contains('1')) s = s.Substring(s.IndexOf('1'));
            //     check++;
            // }
            // return check;
