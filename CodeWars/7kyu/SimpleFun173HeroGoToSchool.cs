/*
https://www.codewars.com/kata/58b63cb2b7d86adb650000b7/csharp
...
namespace CodeWars
{
    public class SimpleFun173HeroGoToSchool
    {
        public int WhichBusToTake(string[] BusesColors, bool[] GoingToSchool)
        {
            return BusesColors.Select((s, i) => (i, s == "red" ? 1 : 2))
                .Where((_, i) => GoingToSchool[i])
                .OrderBy(b => b.Item2)
                .First().i;


            // for (int i = 0; i < BusesColors.Length; i++)
            // {
            //     if (BusesColors[i] == "red" && GoingToSchool[i] == true) return i;
            // }
            //
            // for (int i = 0; i < BusesColors.Length; i++)
            // {
            //     if (BusesColors[i] == "blue" && GoingToSchool[i] == true) return i;
            // }
            //
            // return 0;


            // return BusesColors.Select((s, i) => new {Index = i, Cost = s == "red" ? 1 : 2})
            //     .Where((_, i) => GoingToSchool[i])
            //     .OrderBy(b => b.Cost)
            //     .First().Index;


            // return BusesColors.Zip(GoingToSchool, (b, t) => new {b, t})
            //     .Select((c, i) => new {c.b, c.t, i})
            //     .Where(c => c.t)
            //     .OrderByDescending(c => c.b)
            //     .ThenBy(c => c.i)
            //     .Select(c => c.i)
            //     .First();
        }
