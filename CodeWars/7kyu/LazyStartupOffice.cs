/*
https://www.codewars.com/kata/578fdcfc75ffd1112c0001a1/csharp
...
namespace CodeWars
{
    public class LazyStartupOffice
    {
        public static string[] BinRota(string[][] input)
        {
            return input.SelectMany((x, i) => i % 2 == 0 ? x : x.Reverse()).ToArray();

            // return input.Select((x, i) => i % 2 == 0 ? x : x.Reverse()).SelectMany(x => x).ToArray();
        }
    }
}