/*
https://www.codewars.com/kata/5a0599908ba914a6cf000138/csharp
...
You are given a preloaded dictionary with these 12 notes and one of the pitches that creates that note (starting at Concert A).
...
namespace CodeWars
{
    public class PitchesAndNotes
    {
        ...
        {
            return NotesDictionary.First(x => x.Key % pitch == 0 || pitch % x.Key == 0).Value;
            // return NotesDictionary.Single(e => pitch % e.Key == 0 || e.Key % pitch == 0).Value;
        ...
        private static Dictionary<double, string> NotesDictionary = new Dictionary<double, string>()
        {
            {440, "A"},
            {466.16, "A#"},
            {493.88, "B"},
