/*
https://www.codewars.com/kata/58bf72b02d1c7c18d9000127/csharp
...
In the process of investigating the land, he learned that the state of Louisiana is actually shrinking by 50 square miles each year, 
...
namespace CodeWars
{
    public class SimpleFun187DoesFredNeedHouseboat
    {
        public int DoesFredNeedHouseboat(int x, int y)
        {
            // return (int) Math.Ceiling(.01 * Math.PI * (x * x + y * y));
            // return (int) Math.Ceiling(Math.PI * (x * x + y * y) / 100.0);
            return (int) Math.Ceiling(Math.PI * (x * x + y * y) / 100);
        }
    }
}