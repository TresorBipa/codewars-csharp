/*
https://www.codewars.com/kata/588711735ea0b4649e000001/csharp
...
the second should contain those from 104 + 1 to 2 x 104, ..., the 100th one should contain numbers from 99 x 104 + 1 to 106 and so on.
...
namespace CodeWars
{
    public class SimpleFun34NumbersGrouping
    {
        public int NumbersGrouping(int[] a)
        {
            return a.Length + a.GroupBy(i => (i - 1) / 10000).Count();
            // return a.Count() + a.GroupBy(i => (i - 1) / 10000).Count();
            // return a.GroupBy(x => (x - 1) / 10000).Count() + a.Length;
            // return a.Select(x => Math.Ceiling(x / 10000d)).Distinct().Count() + a.Length;
