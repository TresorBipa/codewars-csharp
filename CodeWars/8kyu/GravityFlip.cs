...
namespace CodeWars
{
    public class GravityFlip
    {
        ...
        {
            return arr.OrderBy(i => dir == 'R' ? i : -i).ToArray();
            // return arr.OrderBy(x => (dir == 'R' ? 1 : -1) * x).ToArray();
            // return (dir == 'R' ? arr.OrderBy(x => x) : arr.OrderBy(x => -x)).ToArray();
            // return dir == 'R' ? arr.OrderBy(i => i).ToArray() : arr.OrderByDescending(i => i).ToArray();
        }
    }
}