...
namespace CodeWars
{
    public class KataGoodVsEvil
    {
        public static string GoodVsEvil(string good, string evil)
        {
            ...
            [
                (good + " " + evil).Split()
                .Zip(new[] {1, 2, 3, 3, 4, 10, -1, -2, -2, -2, -3, -5, -10}, (s, i) => int.Parse(s) * i).Sum()
                .CompareTo(0) + 1
            ];


            // int Calc(int[] forces, string army) => army.Split().Select(int.Parse).Zip(forces, (x, y) => x * y).Sum();
            // var score = Calc(new[] {1, 2, 3, 3, 4, 10}, good) - Calc(new[] {1, 2, 2, 2, 3, 5, 10}, evil);
            // return score == 0 ? "Battle Result: No victor on this battle field" :
            //     score > 0 ? "Battle Result: Good triumphs over Evil" :
            //     "Battle Result: Evil eradicates all trace of Good";


            // int[] goodForces = {1, 2, 3, 3, 4, 10};
            // int[] evilForces = {1, 2, 2, 2, 3, 5, 10};
            // var goodTotal = good.Split().Select((v, i) => int.Parse(v) * goodForces[i]).Sum();
            // var evilTotal = evil.Split().Select((v, i) => int.Parse(v) * evilForces[i]).Sum();
