...
public static class BestTravel
{
    private static IEnumerable<IEnumerable<T>> Combinations<T>(this IEnumerable<T> elements, int k)
    {
        return k == 0
            ? new[] { Array.Empty<T>() }
            : elements.SelectMany((t, i) =>
                elements.Skip(i + 1)
                    .Combinations(k - 1).Select(c => new[] { t }.Concat(c)));
    }

    public static int? chooseBestSum(int t, int k, List<int> ls)
    {
        var result = Combinations(ls, k).Where(x => t >= x.Sum()).ToList();
        return result.Any() ? result.Select(m => m.Sum()).Max() : null;
    }
}
